...
module.exports = {
  ...
  'calories-food': {
    ...
    howToSteps: [
      'Search for a food item or browse categories like grains, pulses, dairy, or snacks',
      'Select the portion size or use the default serving size for each item',
      'View the complete nutritional breakdown including calories, protein, carbs, fat, and fiber',
      'Add multiple items to track total meal calories and macronutrients'
    ],
    methodology: 'Nutritional data is sourced from the Indian Food Composition Table (IFCT-2017) published by the National Institute of Nutrition, Hyderabad, supplemented by FSSAI-compliant food labels. Calorie values are based on the Atwater system (4 cal/g protein, 4 cal/g carbs, 9 cal/g fat). Serving sizes follow standard Indian household portions.',
    faqs: [
  ...
  'speed-dist': {
    ...
    howToSteps: [
      'Select which variable you want to solve for: speed, distance, or time',
      'Enter the two known values with their respective units',
      'View the calculated result instantly with unit conversion options',
      'Use the formula breakdown shown for manual verification or learning'
    ],
    methodology: 'This calculator uses the fundamental kinematic equation: Distance = Speed × Time (d = s × t). Rearranged forms: Speed = Distance / Time, Time = Distance / Speed. Unit conversions use standard SI conversion factors. Results account for consistent unit handling across km/h, m/s, mph, and other speed units.',
    faqs: [
  ...
  'typing-speed': {
    ...
    howToSteps: [
      'Select a test duration (1, 2, or 5 minutes) and difficulty level',
      'Click Start and begin typing the displayed text passage as accurately as possible',
      'The timer counts down while tracking your speed and errors in real time',
      'View your final WPM score, accuracy percentage, and error analysis when time expires'
    ],
    methodology: 'Words per minute (WPM) is calculated using the industry-standard formula: Gross WPM = (Total characters typed / 5) / Minutes elapsed. Net WPM = Gross WPM minus (Uncorrected errors / Minutes). The divisor of 5 represents the average English word length including spaces. Accuracy = (Correct characters / Total characters) × 100.',
    faqs: [
  ...
  'ohm-advanced': {
    ...
    howToSteps: [
      'Select the circuit type: DC (series/parallel) or AC (impedance/power factor)',
      'Enter the known electrical values such as voltage, current, resistance, capacitance, or inductance',
      'View the calculated results including total resistance, impedance, power, and power factor',
      'Use the circuit diagram and formula reference shown alongside results for verification'
    ],
    methodology: 'DC calculations use Ohm\'s Law (V = IR) and Kirchhoff\'s laws. Series resistance: R_total = R₁ + R₂ + ... Parallel: 1/R_total = 1/R₁ + 1/R₂ + ... AC impedance uses Z = √(R² + (X_L - X_C)²) where X_L = 2πfL and X_C = 1/(2πfC). Power factor = cos(φ) = R/Z. All formulas follow IEEE electrical engineering standards.',
    faqs: [
