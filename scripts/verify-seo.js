...
async function run() {
  ...
  // Pre-load all HTML files concurrently
  const htmlContents = new Map();
  await Promise.all(CALCS.map(async function(c) {
    var file = path.join(__dirname, '..', c.slug + '.html');
    try {
      var html = await fsPromises.readFile(file, 'utf8');
      htmlContents.set(c.slug, html);
    } catch (e) {
  ...
  CALCS.forEach(function(c) {
    if (!htmlContents.has(c.slug)) { console.log('  ' + FAIL + ' ' + c.slug + '.html NOT FOUND'); htmlFail++; return; }
    var html = htmlContents.get(c.slug);
  ...
  var dupOk = true;
  CALCS.forEach(function(c) {
    if (!htmlContents.has(c.slug)) return;
    var html = htmlContents.get(c.slug);
  ...
  var financeContent = htmlContents.get('emi-calculator') || '';
  var healthContent = htmlContents.get('bmi-calculator') || '';
  var otherContent = htmlContents.get('discount-calculator') || '';
  check('Finance pages have financial advisor disclaimer', financeContent.indexOf('financial advisor') !== -1);
  check('Health pages have medical disclaimer', healthContent.indexOf('Medical disclaimer') !== -1);
  check('General pages have informational note', otherContent.indexOf('informational purposes') !== -1);

  // ── 6. JSON-LD Validity ────────────────────────────────────────────
  console.log('\n📊 JSON-LD Schema Validity');
  var jsonOk = true;
  ['emi','gratuity','bmi','sleep','ppf','pregnancy','credit-card','grade'].forEach(function(id) {
    var slug = id + '-calculator';
    var html = htmlContents.get(slug);
    if (!html) return;
