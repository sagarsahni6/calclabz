...
REGISTRY.forEach(entry => {
  ...
  if (html.includes('"WebApplication"') || html.includes('"SoftwareApplication"')) {
    skipped++;
    return;
  }

  const name = entry.name || slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  ...
  const desc = descMatch ? descMatch[1] : `Free online ${name}. Instant results, no signup required.`;
