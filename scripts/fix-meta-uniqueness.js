...
async function main() {
  ...
  const promises = REGISTRY.map(async entry => {
    ...
    let html;
    try {
      html = await fsPromises.readFile(fp, 'utf8');
    } catch {
      return;
    }
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const { fp, slug, cat, name, intro, currentDesc, isGeneric } = res;
    let html = res.html;

    // Check for duplicates sequentially to preserve exact same behavior
    ...
    if (currentDesc.length >= 100 && !isGeneric) continue;
    ...
    writePromises.push(fsPromises.writeFile(fp, html, 'utf8'));
    fixed++;
  }
